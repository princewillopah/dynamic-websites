            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ml-auto">
                        ...
                        <!-- About Dropdown -->
                        <li class="nav-item mynavlink dropdown {{(request()->is('about')) || (request()->is('leadership')) || (request()->is('what-we-believe')) ?'active':''}}">
                        ...
                        <li class="nav-item mynavlink dropdown  {{(request()->is('/ministries/'))?'active':''}}">
                         ...
                         <li class="nav-item mynavlink dropdown  {{(request()->is('/our-sermons'))?'active':''}}">
                            <a class="nav-link dropdown-toggle {{(request()->is('/our-sermons'))?'active':''}}" href="{{route('sermons')}}" id="navbardrop" data-toggle="dropdown">
                                Sermons
                            </a>
                            <div class="dropdown-menu"> 
                                <a class="dropdown-item mynavAnchor {{(request()->is('all-sermons'))?'active':''}}" href="{{route('all.sermons')}}">All Sermons
                                    </a>
                                    <a class="dropdown-item mynavAnchor {{(request()->is('our-sermons'))?'active':''}}" href="{{route('sermons')}}">Recent Sermons
                                    </a>
                                  {{--   @foreach($category as $cat)
                                    @if(!empty($cat->sermon))
                                     <a class="dropdown-item mynavAnchor" href="{{route('sermon.category',$cat->id)}}">{{$cat->name}}</a>
                                    @endif
                                   @endforeach  --}} 
                                 {{--  @foreach($category as $cat1)
                                     @foreach($cat1 as $cat)
                                         @if($cat->sermons->count()>0)
                                         <a class="dropdown-item mynavAnchor" href="{{route('sermon.category',$cat1->id)}}">{{$cat1->name}}</a>
                                         @endif 
                                     @endforeach
                                   @endforeach --}}
