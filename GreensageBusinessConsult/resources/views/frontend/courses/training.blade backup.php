   ...
   <div class="row">
       ...
       <form action="">
         ...
         <p>
          ...
          <div class="form-group">
            <input type="checkbox" id="todo" name="todo"  value="{{ $cat->id }}"
            @if (in_array($cat->id, explode(',', request()->input('filter.category'))))
                  checked
              @endif
            >
            <label for="todo" data-content="{{$cat->name}}">{{$cat->name}} ({{$cat->courses->count()}})</label>
          </div>
          @endforeach
         @endif
    ...
    <div class="col-md-8 main-courses-desktop">
      <div class="main-courses">
        <div class="row">
          <div class="title">
            <h5><span style="font-weight: 600">Results Found:</span> {{$format_courses->count()}} of  {{$format_courses_count}} {{$format_courses_count > 1 || 0 ? " Courses":"Course"}}  </h5>
        ...
        {{ $format_courses->appends(['sort' =>$_GET['sort']])->links('vendor.pagination.custom-pagination')}}
        @else
          {{ $format_courses->links('vendor.pagination.custom-pagination')}}
        @endif
